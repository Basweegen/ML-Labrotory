...
impl RelayPanel {
    ...
    /// Evaluates multi-agent votes, extracts objections, and computes consensual alignment score across all DAG nodes
    pub fn compute_dag_consensus_and_votes(&self) -> (f32, Vec<(String, &'static str, egui::Color32)>, Vec<String>) {
        let mut votes = Vec::new();
        let mut objections = Vec::new();
        let mut base_score: f32 = 0.90;

        for node in &self.dag.nodes {
            if !node.status.is_completed() {
                continue;
            }
            let text = node.output.to_lowercase();
            let (verdict, color) = match node.role {
                ModelRole::Critic => {
                    let mut flags = 0;
                    for marker in ["vulnerability", "risk", "flaw", "insecure", "defect", "overflow", "exploit", "leak"] {
                        if text.contains(marker) {
                            flags += 1;
                            objections.push(format!("{}: Flags potential '{}'", node.name, marker));
                        }
                    }
                    if flags > 0 {
                        base_score -= (flags as f32 * 0.05).min(0.25);
                        ("Flagged Concerns", egui::Color32::from_rgb(0xf8, 0x71, 0x71))
                    } else {
                        base_score += 0.04;
                        ("Verified Secure", egui::Color32::from_rgb(0x4a, 0xde, 0x80))
                    }
                }
                ModelRole::Coder => {
                    if node.output.contains("```") || node.output.len() > 150 {
                        base_score += 0.02;
                        ("Code Delivered", egui::Color32::from_rgb(0x38, 0xbd, 0xf8))
                    } else {
                        ("Partial Spec", egui::Color32::from_rgb(0xfb, 0xbf, 0x24))
                    }
                }
                ModelRole::Planner => {
                    base_score += 0.02;
                    ("Architecture Aligned", egui::Color32::from_rgb(0x81, 0x8c, 0xf8))
                }
                _ => ("Approved", egui::Color32::from_rgb(0x34, 0xd3, 0x99)),
            };
            votes.push((node.name.clone(), verdict, color));
        }

        (base_score.clamp(0.20, 0.99), votes, objections)
    }
    ...
    fn show_dag_mode(
        ...
        // Multi-Agent Consensus Debate & Voting Matrix (Phase 5)
        let (consensus_pct, votes, objections) = self.compute_dag_consensus_and_votes();
        if !votes.is_empty() {
            ui.add_space(10.0);
            egui::Frame::group(&ui.style())
                .fill(egui::Color32::from_rgb(0x13, 0x1a, 0x29))
                .stroke(egui::Stroke::new(1.0, egui::Color32::from_rgb(0x2d, 0x3f, 0x5e)))
                .corner_radius(egui::CornerRadius::same(6))
                .inner_margin(egui::Margin::same(10))
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(
                            egui::RichText::new("⚖ Multi-Agent Swarm Voting & Consensus Matrix")
                                .size(13.0)
                                .strong()
                                .color(egui::Color32::from_rgb(0xf5, 0x9e, 0x0b)),
                        );

                        let score_color = if consensus_pct >= 0.85 {
                            egui::Color32::from_rgb(0x10, 0xb9, 0x81)
                        } else if consensus_pct >= 0.65 {
                            egui::Color32::from_rgb(0xfb, 0xbf, 0x24)
                        } else {
                            egui::Color32::from_rgb(0xf8, 0x71, 0x71)
                        };
                        ui.label(
                            egui::RichText::new(format!("Consensus Agreement: {:.1}%", consensus_pct * 100.0))
                                .size(12.0)
                                .strong()
                                .color(score_color),
                        );
                        ...
                        if !objections.is_empty() {
                            ui.label(
                                egui::RichText::new(format!("({} Objections Flagged)", objections.len()))
                                    .size(11.0)
                    ...
                    ui.horizontal_wrapped(|ui| {
                        for (node_name, verdict, color) in &votes {
                            ui.label(
                                egui::RichText::new(format!("{} : {}", node_name, verdict))
                                    .size(11.0)
                                    .color(*color)
                                    .background_color(egui::Color32::from_rgb(0x0f, 0x17, 0x2a)),
                            );
                            ui.add_space(4.0);
                        }
                    });
                    ...
                    if !objections.is_empty() {
                        ...
                        ui.horizontal(|ui| {
                            ui.label(
                                egui::RichText::new("Critique Points:")
                                    .size(11.0)
                                    .color(egui::Color32::from_rgb(0xf8, 0x71, 0x71))
                                    .strong(),
                            );
                            for obj in &objections {
                                ui.label(egui::RichText::new(format!("• {obj}")).size(11.0).color(egui::Color32::from_rgb(0xfc, 0xa5, 0xa5)));
                            }
                        });
                    }
                });
        ...
        egui::Frame::group(&ui.style())
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    // Dynamic Edge Condition Gating Selector (Phase 4)
                    let mut current_cond = self.dag.nodes[node_idx].edge_condition;
                    let cond_color = match current_cond {
                        crate::swarm::dag::EdgeCondition::Always => egui::Color32::from_rgb(0x94, 0xa3, 0xb8),
                        crate::swarm::dag::EdgeCondition::OnSuccess => egui::Color32::from_rgb(0x10, 0xb9, 0x81),
                        crate::swarm::dag::EdgeCondition::OnFailure => egui::Color32::from_rgb(0xf5, 0x9e, 0x0b),
                    };
                    egui::ComboBox::from_id_salt(format!("dag_node_cond_{}", node_id))
                        .selected_text(egui::RichText::new(current_cond.badge()).size(10.0).color(cond_color))
                        .width(90.0)
                        .show_ui(ui, |ui| {
                            if ui.selectable_value(&mut current_cond, crate::swarm::dag::EdgeCondition::Always, "➜ Always").clicked() {
                                self.dag.nodes[node_idx].edge_condition = crate::swarm::dag::EdgeCondition::Always;
                            }
                            if ui.selectable_value(&mut current_cond, crate::swarm::dag::EdgeCondition::OnSuccess, "✔ If Pass").clicked() {
                                self.dag.nodes[node_idx].edge_condition = crate::swarm::dag::EdgeCondition::OnSuccess;
                            }
                            if ui.selectable_value(&mut current_cond, crate::swarm::dag::EdgeCondition::OnFailure, "✖ If Fail").clicked() {
                                self.dag.nodes[node_idx].edge_condition = crate::swarm::dag::EdgeCondition::OnFailure;
                            }
                        });
